  ...
  for (int k = 0; k < M; k++) {
    for (int i = N - 1; i >= 0; i--) {
      ...
      if (MATRIX_AT(a, i, i) == 0) {
        MATRIX_ERROR("Diagonal elements of left matrix must not be zero", E_MATRIX_DIVZERO);
      }
...
void matrix_inv(Matrix *dst, const Matrix *src) {
  ...
  if (src->col != N) {
    ...
    MATRIX_ERROR("Destination and source matrix must have equal size", E_MATRIX_NEQSIZ);
  }
...
void matrix_lu_dcmp(Matrix *l, Matrix *u, Matrix *p, const Matrix *src) {
  ...
  } else if (l->row != N || l->col != N || u->row != N || u->col != N
             || p->row != N || p->col != N) {
    MATRIX_ERROR("Decomposition matrices must have the same size as the source", E_MATRIX_NEQSIZ);
  }
...
double matrix_lu_det(const Matrix *l, const Matrix *u, const Matrix *p) {
  ...
  if (l->col != N || u->row != N || u->col != N || p->row != N || p->col != N) {
    MATRIX_ERROR("All matrices passed must be square and have the same size", E_MATRIX_NEQSIZ);
  }
                     ...
                     const Matrix *p, const Matrix *b)
{
  ...
  if (dst->row != N || dst->col != M) {
    MATRIX_ERROR("Invalid size for solution vector", E_MATRIX_NGPTR);
             ...
             || p->row != N || p->col != N) {
    MATRIX_ERROR("L, U, P matrices must be square and have the same size", E_MATRIX_NEQSIZ);
  }
