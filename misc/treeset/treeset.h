...
#define treeset_deref(type, node) \
  (*((type *)((node) + 1)))

#define treeset_size(set) \
  (rb_size(&((set)->tree)))

#define treeset_clear(set) \
  (rb_clear(&((set)->tree)))

#define treeset_first(set) \
  (rb_first(&((set)->tree)))

#define treeset_last(set) \
  (rb_last(&(set)->tree))

#define treeset_prev(node) \
  (rb_prev(node))

#define treeset_next(node) \
  (rb_next(node))
