...
void DataLoader::forward(bool is_train) {
    if (is_train) {
        // update offset
        int start = train_data_offset;
        int end = std::min(start + batch_size,
                           (int)dataset->get_train_images().size());
        ...
        std::fill(output_labels->get_vec().begin(),
                  output_labels->get_vec().end(), 0);
        ...
        for (int i = start; i < end; i++) {
            ...
            int one_hot_index = (i - start) * n_labels +
                                (int)(dataset->get_train_labels()[i]);
        ...
        int end = std::min(start + batch_size,
                           (int)dataset->get_test_images().size());
        ...
        for (int i = start; i < end; i++) {
            ...
            int one_hot_index = (i - start) * n_labels +
                                (int)(dataset->get_test_labels()[i]);
            output_labels->get_vec()[one_hot_index] = 1;
        }
    }
}

bool DataLoader::has_next(bool is_train) {
    if (is_train) {
