...
public:
    ...
    void forward(const MatXf &prev_out, bool is_training) override {
        ...
        for (int n = 0; n < batch; n++) {
            ...
            const float *begin = prev_out.data() + offset;
            float max = *std::max_element(begin, begin + height);
            float log_sum = std::log(sum_exp(begin, height, max));
            log_sum += max;
            std::transform(begin, begin + height, output.data() + offset,
                           [&](const float &e) { return e - log_sum; });
        }
    }
    ...
    void backward(const MatXf &prev_out, MatXf &prev_delta) override {
        for (int n = 0; n < batch; n++) {
            int offset = height * n;
            const float *begin_op = prev_out.data() + offset;
            float max = *std::max_element(begin_op, begin_op + height);
            float log_sum = std::log(sum_exp(begin_op, height, max));
            log_sum += max;
            const float *begin_d = delta.data() + offset;
            float *begin_dp = prev_delta.data() + offset;
            float dldy_sum = std::accumulate(begin_d, begin_d + height, 0.f);
            std::transform(begin_d, begin_d + height, begin_op, begin_dp,
                           [&](const float &e1, const float &e2) {
                               return e1 - dldy_sum * std::exp(e2 - log_sum);
                           });
        }
    }
};
